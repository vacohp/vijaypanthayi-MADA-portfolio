...
toptendeath <- read_csv(here("dataanalysis-exercise","data","raw_data","NCHS_-_Leading_Causes_of_Death__United_States.csv"))
glimpse(toptendeath)
```
...
glimpse(ga_suicide)
```
Now that the data is cleaned and only contains the yarly suicide deaths in Georgia from 1999 - 2017, we can save this new data set.
```{r}
ga_suicide %>% saveRDS(here::here("dataanalysis-exercise", "data", "processed_data","ga_suicide.rds"))
```
